"""
File adapted from learn2branch repo
"""
import os
import argparse
import numpy as np
...
if __name__ == '__main__':
    ...
    if args.problem == 'setcover':
        ...
        nrows = 1000
        ncols = 1000
        lp_dir = f'data/instances/setcover/test_medium_{nrows}r_{ncols}c_{dens}d'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nrowss.extend([nrows] * n)
        ...
        n = 2000
        number_of_nodes = 1000
        lp_dir = f'data/instances/indset/test_medium_{number_of_nodes}_{affinity}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nnodess.extend([number_of_nodes] * n)
        ...
        number_of_items = 200
        number_of_bids = 1000
        lp_dir = f'data/instances/cauctions/test_medium_{number_of_items}_{number_of_bids}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nitemss.extend([number_of_items] * n)
        ...
        # test medium instances
        n = 100
        number_of_customers = 200
        number_of_facilities = 100
        lp_dir = f'data/instances/facilities/test_medium_{number_of_customers}_{number_of_facilities}_{ratio}'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ncustomerss.extend([number_of_customers] * n)
